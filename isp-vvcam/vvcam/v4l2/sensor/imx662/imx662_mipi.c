...
static int imx662_power_off(struct imx662 *sensor)
{
	...
	if (err < 0) {
		pr_err("%s: failed to sett XVS XHS to Hi-Z\n", __func__);
	}
...
static int imx662_set_black_level(struct imx662 *sensor, s64 val, u32 which_control)
{
	int ret = 0;
	s64 black_level_reg;
	// TODO: add which control logic, how is black level set inside ISI driver?
...
static int imx662_set_test_pattern(struct imx662 *sensor, u32 pattern)
{
	...
	if (0 < pattern  && pattern < ARRAY_SIZE(imx662_test_pattern_menu)) {
...
static struct v4l2_subdev_video_ops imx662_subdev_video_ops = {
