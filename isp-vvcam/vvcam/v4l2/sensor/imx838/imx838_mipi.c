 ...
 *
 * imx838_mipi.c - Framos imx838_mipi.c driver
 */
...
#include "vvsensor.h"
#include "imx838_regs.h"
#include "max96792.h"
#include "max96793.h"

#define IMX838_MAX_RETRIES 10

#define IMX838_SENS_PAD_SOURCE	0
#define IMX838_SENS_PADS_NUM	1
...
#define IMX838_GAIN_RATIO  (IMX838_MAX_GAIN_DB * 10 / IMX838_MAX_GAIN_DEC)
#define IMX838_HIGH_GAIN_REG_MIN 34
#define IMX838_HIGH_GAIN_MIN (IMX838_HIGH_GAIN_REG_MIN * IMX838_GAIN_RATIO) 
...
#define IMX838_MIN_SHR0_LENGTH 9
#define IMX838_MIN_SHR0_CLEAR_LENGTH     8
#define IMX838_MIN_SHR0_RHS1_DIST        5
#define IMX838_MIN_SHR1_LENGTH           5
#define IMX838_MIN_INTEGRATION_LINES     10
#define IMX838_MAX_VS_INTEGRATION_LINES  66
#define IMX838_MIN_VS_INTEGRATION_LINES  10
#define IMX838_BRL                       2200
#define IMX838_10BIT_INTEGRATION_OFFSET 1
#define IMX838_12BIT_INTEGRATION_OFFSET 2

#define IMX838_MAX_CSI_LANES 4
#define IMX838_TWO_LANE_MODE 1
#define IMX838_FOUR_LANE_MODE 3

#define IMX838_1ST_INCK 74250000LL
#define IMX838_2ND_INCK 72000000LL

#define IMX838_XCLK_MIN 37000000
#define IMX838_XCLK_MAX 37250000

#define IMX838_MAX_BOUNDS_WIDTH 3856
#define IMX838_MAX_BOUNDS_HEIGHT 2250
#define IMX838_LINE_TIME 14814 /* in ns */
...
enum mode_index {
	IMX838_ALL_PIXEL_INDEX,
	IMX838_BINNING_INDEX,
	IMX838_DOL_INDEX,
	IMX838_CLEAR_INDEX,
	IMX838_MAX_INDEX
};
 ...
 */
#define IMX838_GAIN_REG_LEN 241
static const u32 gain_reg2times[IMX838_GAIN_REG_LEN] = {
...
static const struct v4l2_ctrl_ops imx838_ctrl_ops;
static struct v4l2_ctrl_config imx838_ctrl_data_rate[] = {
	{
		.ops = &imx838_ctrl_ops,
		.id = V4L2_CID_DATA_RATE,
		.name = "Data rate",
		.type = V4L2_CTRL_TYPE_INTEGER,
		.min = IMX838_2376_MBPS,
		.max = IMX838_594_MBPS,
		.def = IMX838_891_MBPS,
		.step = 1,
	},
};
static struct v4l2_ctrl_config imx838_ctrl_sync_mode[] = {
	{
		.ops = &imx838_ctrl_ops,
...
static struct v4l2_ctrl_config imx838_ctrl_framerate[] = {
	{
		.ops = &imx838_ctrl_ops,
...
static struct v4l2_ctrl_config imx838_ctrl_vs_exp[] = {
	{
		.ops = &imx838_ctrl_ops,
		.id = V4L2_CID_VS_EXP,
		.name = "VS exposure",
...
static struct v4l2_ctrl_config imx838_ctrl_vs_gain[] = {
	{
		.ops = &imx838_ctrl_ops,
		.id = V4L2_CID_VS_GAIN,
		.name = "VS gain",
...
static struct v4l2_ctrl_config imx838_ctrl_exp_gain[] = {
	{
		.ops = &imx838_ctrl_ops,
...
static struct v4l2_ctrl_config imx838_ctrl_conv_gain[] = {
	{
		.ops = &imx838_ctrl_ops,
...
static struct v4l2_ctrl_config imx838_ctrl_conv_vs_gain[] = {
	{
		.ops = &imx838_ctrl_ops,
...
struct imx838 {
	...
	struct media_pad pads[IMX838_SENS_PADS_NUM];
...
#define client_to_imx838(client)\
	container_of(i2c_get_clientdata(client), struct imx838, sd)

static inline struct imx838 *to_imx838_dev(struct v4l2_subdev *sd)
{
	return container_of(sd, struct imx838, sd);
}

static inline struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct imx838, ctrls.handler)->sd;
}

static struct vvcam_mode_info_s pimx838_mode_info[] = {
	{
		.index          = IMX838_ALL_PIXEL_INDEX,
		.size           = {
			.bounds_width  = IMX838_DEFAULT_WIDTH,
			.bounds_height = IMX838_DEFAULT_HEIGHT,
			.top           = 12,
			.left          = 8,
			.width         = 3840,
			.height        = 2160,
		...
		.ae_info = {
			.def_frm_len_lines     = IMX838_MAX_BOUNDS_HEIGHT,
			.curr_frm_len_lines    = IMX838_MAX_BOUNDS_HEIGHT,
			.one_line_exp_time_ns  = IMX838_LINE_TIME,

			.max_integration_line  = IMX838_MAX_BOUNDS_HEIGHT - 1,
		...
		.preg_data      = imx838_init_setting,
		.reg_data_count = ARRAY_SIZE(imx838_init_setting),
	},
	{
		.index          = IMX838_BINNING_INDEX,
		.size           = {
			.bounds_width  = IMX838_MODE_BINNING_H2V2_WIDTH,
			.bounds_height = IMX838_MODE_BINNING_H2V2_HEIGHT,
			.top           = 6,
			.left          = 4,
			.width         = 1920,
			.height        = 1080,
	...
	{
		.index          = IMX838_DOL_INDEX,
		.size           = {
			.bounds_width  = IMX838_DEFAULT_WIDTH,
			.bounds_height = IMX838_DEFAULT_HEIGHT,
			.top           = 0,
			.left          = 0,
			.width         = IMX838_DEFAULT_WIDTH,
			.height        = IMX838_DEFAULT_HEIGHT,
		},
		...
		.ae_info = {
			...
			.max_vsintegration_line = IMX838_MAX_VS_INTEGRATION_LINES,
			.min_vsintegration_line = IMX838_MIN_VS_INTEGRATION_LINES,

			// reduce max integration line for short exposure to allow vs exposure
			.max_integration_line  = 2 * IMX838_MAX_BOUNDS_HEIGHT - IMX838_MAX_VS_INTEGRATION_LINES,
			.min_integration_line  = IMX838_MIN_INTEGRATION_LINES,
			...
			.start_exposure        = 1000 * 1024, // 10000 * 1024,
			.cur_fps               = 15 * 1024,
			.max_fps               = 15 * 1024,
			.min_fps               = 1 * 1024,
			.min_afps              = 1 * 1024,
	...
	},
	{
		...
		.bayer_pattern = BAYER_RGGB,
		.ae_info = {
			.def_frm_len_lines     = IMX838_MAX_BOUNDS_HEIGHT * 2,
			.curr_frm_len_lines    = IMX838_MAX_BOUNDS_HEIGHT * 2,
			.one_line_exp_time_ns  = IMX838_LINE_TIME,

			.max_integration_line  = IMX838_MAX_BOUNDS_HEIGHT * 2 - 4,
			...
			.start_exposure        = 1000 * 1024, // 30000 * 1024,
			.cur_fps               = 15 * 1024,
			.max_fps               = 15 * 1024,
			.min_fps               = 1 * 1024,
			.min_afps              = 1 * 1024,
...
static int imx838_write_reg(struct imx838 *sensor, u16 reg, u8 val)
{
	 ...
	 * Retry sending a message for IMX838_MAX_RETRIES and report a problem.
	 */
	for (num_retry = 0; num_retry < IMX838_MAX_RETRIES; num_retry++) {
...
static int imx838_read_reg(struct imx838 *sensor, u16 reg, u8 *val)
{
	struct device *dev = &sensor->i2c_client->dev;
	u8 au8RegBuf[2] = { 0 };
	u8 u8RdVal = 0;
 ...
 * Retry sending a message for IMX838_MAX_RETRIES and report a problem.
 */
static int imx838_i2c_transfer(const struct i2c_client *const i2c_client,
				u8 *send_buf,
...
static int imx838_write_reg_arry(struct imx838 *sensor,
				 ...
				 u32 size)
{
	...
	for (i = 1; i < size; i++) {
		...
		if ((send_buf_len < max_send_buff) && (reg_arry[i].addr == (reg_arry[i-1].addr + 1))) {
			...
			ret = imx838_i2c_transfer(i2c_client, send_buf, send_buf_len);
			if (ret < 0) {
				kfree(send_buf);
				return ret;
			}
...
static int imx838_power_on(struct imx838 *sensor)
{
...
static int imx838_power_off(struct imx838 *sensor)
{
	pr_debug("enter %s function\n", __func__);
...
static int imx838_s_power(struct v4l2_subdev *sd, int on)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct imx838 *sensor = client_to_imx838(client);
	int ret = 0;

	pr_debug("enter %s function\n", __func__);
	if (on)
		ret = imx838_power_on(sensor);
	else
		ret = imx838_power_off(sensor);
...
static int imx838_query_capability(struct imx838 *sensor, void *arg)
{
	struct v4l2_capability *pcap = (struct v4l2_capability *)arg;

	strscpy((char *)pcap->driver, "imx838", sizeof(pcap->driver));
...
static int imx838_query_supports(struct imx838 *sensor, void *parry)
{
	int ret = 0;
	struct vvcam_mode_info_array_s *psensor_mode_arry = parry;
	uint32_t support_counts = ARRAY_SIZE(pimx838_mode_info);

	pr_debug("enter %s function\n", __func__);

	ret = copy_to_user(&psensor_mode_arry->count, &support_counts,
			sizeof(support_counts));
	ret |= copy_to_user(&psensor_mode_arry->modes, pimx838_mode_info,
			   sizeof(pimx838_mode_info));
	if (ret != 0) {
...
static int imx838_get_sensor_id(struct imx838 *sensor, void *pchip_id)
{
	int ret = 0;
	u16 chip_id = 838;
...
static int imx838_get_reserve_id(struct imx838 *sensor, void *preserve_id)
{
	int ret = 0;
	u16 reserve_id = 838;
...
static int imx838_get_sensor_mode(struct imx838 *sensor, void *pmode)
{
	int ret = 0;
...
static int imx838_set_sensor_mode(struct imx838 *sensor, void *pmode)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < ARRAY_SIZE(pimx838_mode_info); i++) {
		if (pimx838_mode_info[i].index == sensor_mode.index) {
			memcpy(&sensor->cur_mode, &pimx838_mode_info[i],
				sizeof(struct vvcam_mode_info_s));
			return 0;
		}
	}
 ...
 */
static int imx838_adjust_hmax_register(struct imx838 *sensor)
{
	int err = 0;
	u32 hmax = 628;
	...
	err = imx838_read_reg(sensor, DATARATE_SEL, &data_rate);
	err |= imx838_read_reg(sensor, ADDMODE, &current_binning_mode);
	...
	err = imx838_write_reg(sensor, REGHOLD, 1);
	err |= imx838_write_reg(sensor, HMAX_HIGH, (hmax >> 8) & 0xff);
	err |= imx838_write_reg(sensor, HMAX_LOW, hmax & 0xff);
	err |= imx838_write_reg(sensor, REGHOLD, 0);
	if (err) {
		pr_err("%s: failed to set HMAX register\n", __func__);
		return err;
	}

	sensor->cur_mode.ae_info.one_line_exp_time_ns = (hmax*IMX838_G_FACTOR) / IMX838_1ST_INCK;
...
static int imx838_change_data_rate(struct imx838 *sensor, u32 data_rate)
{
	int ret = 0;
	struct device *dev = &sensor->i2c_client->dev;
	...
	ret = imx838_read_reg(sensor, LANEMODE, &current_lane_mode);

	if (current_lane_mode == IMX838_TWO_LANE_MODE) {
		pr_info("%s: 2 lane mode is not supported, switching to 4 lane mode\n",
		__func__);
		imx838_write_reg(sensor, LANEMODE, IMX838_FOUR_LANE_MODE);
	}

	ret |= imx838_read_reg(sensor, ADDMODE, &current_binning_mode);
	if (ret < 0) {
	...
	ret = imx838_write_reg(sensor, DATARATE_SEL, data_rate);
	return ret;
...
static int imx838_set_data_rate(struct imx838 *sensor, u32 data_rate)
{
	int ret = 0;

	pr_info("enter %s data rate received: %u\n", __func__, data_rate);

	ret = imx838_change_data_rate(sensor, data_rate);
	if (ret)
		goto fail;

	ret = imx838_adjust_hmax_register(sensor);
	if (ret) {
 ...
 * 0xF - hi-z,   hi-z
 */
static int imx838_configure_triggering_pins(struct imx838 *sensor)
{
	int err = 0;
	u8 extmode;
	u8  xvs_xhs_drv = 0xF;
	...
	err = imx838_read_reg(sensor, EXTMODE, &extmode);
	...
	err = imx838_write_reg(sensor, XVS_XHS_DRV, xvs_xhs_drv);
	if (err) {
 ...
 */
static int imx838_set_sync_mode(struct imx838 *sensor, u32 val)
{
	int err = 0;
	u8 extmode = 0;
	...
	if (sensor->powered_on == 1) {
		err = imx838_write_reg(sensor, EXTMODE, extmode);
		if (err < 0)
			pr_err("%s: error setting sync mode\n", __func__);
	}

	err = imx838_configure_triggering_pins(sensor);
	if (err < 0) {
...
static int imx838_set_exp(struct imx838 *sensor, u32 exp, unsigned int which_control)
{
	int ret = 0;
	...
	if (which_control == 0) {
		...
		* IMX838_K_FACTOR) / sensor->cur_mode.ae_info.one_line_exp_time_ns;
	}
	/* from V4L2 control */
	else {
		pr_debug("enter %s exposure received: %u\n", __func__, exp);
		integration_time_line = exp * IMX838_K_FACTOR / sensor->cur_mode.ae_info.one_line_exp_time_ns;
	}
	...
	if (sensor->cur_mode.index == IMX838_DOL_INDEX) {
		...
		if (reg_shr0 > (2 * frame_length - IMX838_MIN_INTEGRATION_LINES)) {
			pr_err("%s reg_shr0 too large: %u\n", __func__, reg_shr0);
			reg_shr0 = 2 * frame_length - IMX838_MIN_INTEGRATION_LINES;
			pr_err("%s setting reg_shr0 to : %u\n", __func__, reg_shr0);
	}
	} else {
		reg_shr0 = frame_length - integration_time_line;
	if (reg_shr0 < IMX838_MIN_SHR0_LENGTH)
		reg_shr0 = IMX838_MIN_SHR0_LENGTH;
		else if (reg_shr0 > (frame_length - IMX838_MIN_INTEGRATION_LINES))
			reg_shr0 = frame_length - IMX838_MIN_INTEGRATION_LINES;
	}

	// Sanity check
	switch (sensor->cur_mode.index) {
	case IMX838_CLEAR_INDEX:
		min_shr0 = IMX838_MIN_SHR0_CLEAR_LENGTH;
		break;
	default:
		min_shr0 = IMX838_MIN_SHR0_LENGTH;
		break;
	}
	...
	ret = imx838_write_reg(sensor, REGHOLD, 1);
	ret |= imx838_write_reg(sensor, SHR0_HIGH, (reg_shr0 >> 16) & 0xff);
	ret |= imx838_write_reg(sensor, SHR0_MID, (reg_shr0 >> 8) & 0xff);
	ret |= imx838_write_reg(sensor, SHR0_LOW, reg_shr0 & 0xff);
	ret |= imx838_write_reg(sensor, REGHOLD, 0);
...
static int imx838_get_exp_register(struct imx838 *sensor, u32 *reg_shr0)
{
	int ret = 0;
	u8 val = 0;

	ret = imx838_read_reg(sensor, SHR0_HIGH, &val);
	*reg_shr0 = val;
	ret |= imx838_read_reg(sensor, SHR0_MID, &val);
	*reg_shr0 = (*reg_shr0 << 8) + val;
	ret |= imx838_read_reg(sensor, SHR0_LOW, &val);
	*reg_shr0 = (*reg_shr0 << 8) + val;

	return ret;
}

static int imx838_set_vs_exp(struct imx838 *sensor, u32 exp, unsigned int which_control)
{
	int ret = 0;
	u32 reg_shr0 = 0;
	u32 integration_time_line;
	u32 frame_length;
	u32 reg_rhs1 = 5;
	u32 reg_shr1 = IMX838_MIN_SHR1_LENGTH;

	pr_debug("enter %s vs exposure received: %u\n", __func__, exp);

	ret = imx838_get_exp_register(sensor, &reg_shr0);
	if (ret < 0) {
	...
	reg_rhs1 = reg_shr0 - IMX838_MIN_SHR0_RHS1_DIST;
	reg_rhs1 = max_t(u32, 2 * IMX838_BRL - 1, reg_rhs1);
	...
	ret = imx838_write_reg(sensor, REGHOLD, 1);
	ret |= imx838_write_reg(sensor, SHR1_LOW, reg_shr1);
	ret |= imx838_write_reg(sensor, RHS1_LOW, reg_rhs1 & 0xff);
	ret |= imx838_write_reg(sensor, RHS1_MID, (reg_rhs1 >> 8) & 0xff);
	ret |= imx838_write_reg(sensor, RHS1_HIGH, (reg_rhs1 >> 16) & 0xff);
	ret |= imx838_write_reg(sensor, REGHOLD, 0);
 ...
 */
static u32 imx838_get_gain_reg(u32 gain)
{
	u32 l = 0;
	u32 r = IMX838_GAIN_REG_LEN - 1;
	u32 mid;
	u32 ret = 0;
	...
	if (gain < gain_reg2times[0]) {
		pr_warn("%s: isp returned too small gain value: %u, setting to min gain\n",
			__func__, gain);
		return 0;
	} else if (gain > gain_reg2times[IMX838_GAIN_REG_LEN-1]) {
...
static int imx838_set_gain(struct imx838 *sensor, u32 gain, unsigned int which_control)
{
	int ret = 0;
	u32 gain_reg = 0;
	...
	if (which_control == 0) { /* from isp */
		gain_reg = imx838_get_gain_reg(gain);
	} else { /* from v4l2 control */
		gain_reg = gain / IMX838_GAIN_RATIO;
	}

	if (sensor->cur_mode.index == IMX838_CLEAR_INDEX) {
	...
	*/
	if (sensor->ctrls.conv_gain->val == 1) {
		pr_warn("%s: gain value too small for high gain setting value to %d \n", __func__, IMX838_HIGH_GAIN_REG_MIN);
		if (gain_reg < IMX838_HIGH_GAIN_REG_MIN) {
			gain_reg = IMX838_HIGH_GAIN_REG_MIN;
		}
	}

	pr_debug("%s: gain register: %u\n", __func__, gain_reg);
	ret = imx838_write_reg(sensor, REGHOLD, 1);
	ret |= imx838_write_reg(sensor, GAIN_0_HIGH, (gain_reg>>8) & 0xff);
	ret |= imx838_write_reg(sensor, GAIN_0_LOW, gain_reg & 0xff);
	ret |= imx838_write_reg(sensor, REGHOLD, 0);

	return ret;
}

static int imx838_get_low_gain(struct imx838 *sensor, u32 *reg_gain)
{
	int ret = 0;
	u8 val = 0;

	ret = imx838_read_reg(sensor, GAIN_0_HIGH, &val);
	*reg_gain = (*reg_gain << 8) + val;
	ret |= imx838_read_reg(sensor, GAIN_0_LOW, &val);
	*reg_gain = (*reg_gain << 8) + val;

	return ret;
}

static int imx838_get_high_gain(struct imx838 *sensor, u32 *reg_gain)
{
	int ret = 0;
	u8 val = 0;

	ret = imx838_read_reg(sensor, GAIN_1_HIGH, &val);
	*reg_gain = (*reg_gain << 8) + val;
	ret |= imx838_read_reg(sensor, GAIN_1_LOW, &val);
	*reg_gain = (*reg_gain << 8) + val;

	return ret;
}

static int imx838_set_vs_gain(struct imx838 *sensor, u32 gain, u8 which_control)
{
	int ret = 0;
	u32 gain_reg;
	...
	if (which_control == 0) { // from isp
		...
		gain_reg = gain * IMX838_MAX_GAIN_DEC /
				 (IMX838_MAX_GAIN_DB * 10);
	}

	// when vs gain is too large lines occurs on a screen
	if (gain_reg > max_vs_gain) {
		gain_reg = max_vs_gain;
		pr_warn("%s: gain register too large, setting gain register to: %u\n", __func__, gain_reg);
	}
	...
	*/
	...
	ret = imx838_write_reg(sensor, REGHOLD, 1);
	ret |= imx838_write_reg(sensor, GAIN_1_HIGH, (gain_reg>>8) & 0xff);
	ret |= imx838_write_reg(sensor, GAIN_1_LOW, gain_reg & 0xff);
	ret |= imx838_write_reg(sensor, REGHOLD, 0);

	return ret;
}

static int imx838_set_exp_gain(struct imx838 *sensor, u32 gain, u8 which_control)
{
	int ret = 0;
	u32 gain_reg;
	...
	pr_debug("%s: gain register: %u\n", __func__, gain_reg);
	ret = imx838_write_reg(sensor, EXP_GAIN, gain_reg);
	if (ret < 0) {
...
static int imx838_set_black_level(struct imx838 *sensor, s64 val, u32 which_control)
{
	int ret = 0;
	s64 black_level_reg;
	...
	ret = imx838_write_reg(sensor, REGHOLD, 1);
	ret |= imx838_write_reg(sensor, BLKLEVEL_HIGH, (black_level_reg>>8) & 0xff);
	ret |= imx838_write_reg(sensor, BLKLEVEL_LOW, black_level_reg & 0xff);
	ret |= imx838_write_reg(sensor, REGHOLD, 0);
	if (ret) {
...
static int imx838_set_conv_gain(struct imx838 *sensor, u32 val)
{
	int ret = 0;
	u32 curr_gain;
	...
	ret = imx838_write_reg(sensor, FDG_SEL0, val);
	if (ret) {
		pr_err("%s: Error setting conversion gain\n", __func__);
		return ret;
	}
	if (val == 1) {
		ret = imx838_get_low_gain(sensor, &curr_gain);
		curr_gain = curr_gain * IMX838_GAIN_RATIO; 
		curr_gain = max(IMX838_HIGH_GAIN_MIN, curr_gain);
		ret |= imx838_set_gain(sensor, curr_gain, 1);

		sensor->cur_mode.ae_info.min_again = 
				gain_reg2times[IMX838_HIGH_GAIN_REG_MIN];
		if (ret) {
...
static int imx838_set_conv_vs_gain(struct imx838 *sensor, u32 val)
{
	int ret = 0;
	u32 curr_gain;
	pr_info("enter %s conv gain: %u\n",  __func__, val);
	...
	ret = imx838_write_reg(sensor, FDG_SEL1, val);
	if (ret) {
		pr_err("%s: Error setting conversion gain\n", __func__);
		return ret;
	}
	if (val == 1) {
		ret = imx838_get_high_gain(sensor, &curr_gain);
		curr_gain = curr_gain * IMX838_GAIN_RATIO; 
		curr_gain = max(IMX838_HIGH_GAIN_MIN, curr_gain);
		ret |= imx838_set_vs_gain(sensor, curr_gain, 1);
...
static int imx838_set_fps(struct imx838 *sensor, u32 fps, u32 which_control)
{
	u32 fps_reg;
	u32 line_time;
	int ret = 0;
	...
	fps_reg = IMX838_G_FACTOR / ((fps >> 10) * line_time);
	if (sensor->cur_mode.index == IMX838_DOL_INDEX) {
		// in dol case there are two times more lines to read
		fps_reg = fps_reg / 2;
	}
	/* Value must be multiple of 2  TODO: check this */
	fps_reg = (fps_reg % 2) ? fps_reg + 1 : fps_reg;
	pr_debug("enter %s vmax register: %u\n", __func__, fps_reg);
	ret = imx838_write_reg(sensor, REGHOLD, 1);
	ret |= imx838_write_reg(sensor, VMAX_HIGH, (u8)(fps_reg >> 16) & 0xff);
	ret |= imx838_write_reg(sensor, VMAX_MID, (u8)(fps_reg >> 8) & 0xff);
	ret |= imx838_write_reg(sensor, VMAX_LOW, (u8)(fps_reg & 0xff));
	ret |= imx838_write_reg(sensor, REGHOLD, 0);
	...
	else if (sensor->cur_mode.index == IMX838_CLEAR_INDEX)
...
static int imx838_get_fps(struct imx838 *sensor, u32 *pfps)
{
	pr_debug("enter %s function\n", __func__);
...
static int imx838_set_test_pattern(struct imx838 *sensor, u32 pattern)
{
	int ret;

	pr_debug("enter %s, pattern = %u\n", __func__, pattern);
	if (pattern > 0 && pattern < ARRAY_SIZE(test_pattern_menu)) {
		ret = imx838_write_reg_arry(sensor,
			(struct vvcam_sccb_data_s *)mode_enable_pattern_generator,
			ARRAY_SIZE(mode_enable_pattern_generator));
		if (ret < 0) {
			pr_err("%s:imx838_write_reg_arry error\n", __func__);
			return -EINVAL;
		}
		ret = imx838_write_reg(sensor, TPG_PATSEL_DUOUT, pattern - 1);
	} else {
		ret = imx838_write_reg_arry(sensor,
...
static int imx838_set_ratio(struct imx838 *sensor, void *pratio)
{
	int ret = 0;
...
static int imx838_s_ctrl(struct v4l2_ctrl *ctrl)
{
	struct v4l2_subdev *sd = ctrl_to_sd(ctrl);
	struct imx838 *sensor = to_imx838_dev(sd);
	int ret;
	...
	switch (ctrl->id) {
	case V4L2_CID_GAIN:
		ret = imx838_set_gain(sensor, ctrl->val, 1);
		break;
	case V4L2_CID_EXPOSURE:
		ret = imx838_set_exp(sensor, ctrl->val, 1);
		break;
	case V4L2_CID_TEST_PATTERN:
		ret = imx838_set_test_pattern(sensor, ctrl->val);
		break;
	case V4L2_CID_FRAME_RATE:
		ret = imx838_set_fps(sensor, ctrl->val, 1);
		break;
	case V4L2_CID_BLACK_LEVEL:
		ret = imx838_set_black_level(sensor, ctrl->val, 1);
		break;
	case V4L2_CID_DATA_RATE:
		ret = imx838_set_data_rate(sensor, ctrl->val);
		break;
	case V4L2_CID_SYNC_MODE:
		ret = imx838_set_sync_mode(sensor, ctrl->val);
		break;
	case V4L2_CID_VS_EXP:
		ret = imx838_set_vs_exp(sensor, ctrl->val, 1);
		break;
	case V4L2_CID_VS_GAIN:
		ret = imx838_set_vs_gain(sensor, ctrl->val, 1);
		break;
	case V4L2_CID_EXP_GAIN:
		ret = imx838_set_exp_gain(sensor, ctrl->val, 1);
		break;
	case V4L2_CID_CONV_GAIN:
		ret = imx838_set_conv_gain(sensor, ctrl->val);
		break;
	case V4L2_CID_CONV_VS_GAIN:
		ret = imx838_set_conv_vs_gain(sensor, ctrl->val);
		break;
	default:
		ret = -EINVAL;
		break;
...
static const struct v4l2_ctrl_ops imx838_ctrl_ops = {
	.s_ctrl = imx838_s_ctrl,
};

static int imx838_get_format_code(struct imx838 *sensor, u32 *code)
{
	pr_debug("enter %s function\n", __func__);
...
static int imx838_parse_dt(struct imx838 *sensor, struct i2c_client *client)
{
	...
	match = of_match_device(imx838_of_match, &client->dev);
	if (!match) {
...
static int imx838_set_pixel_format(struct imx838 *sensor)
{
	int err;

	if ((sensor->cur_mode.bit_width == 10) && (sensor->cur_mode.index == IMX838_CLEAR_INDEX))
		err = imx838_write_reg_arry(
			sensor,
			(struct vvcam_sccb_data_s *)imx838_10bit_mode_clearHDR,
			ARRAY_SIZE(imx838_10bit_mode_clearHDR));
	else if (sensor->cur_mode.bit_width == 10)
		err = imx838_write_reg_arry(
			sensor,
			(struct vvcam_sccb_data_s *)imx838_10bit_mode,
			ARRAY_SIZE(imx838_10bit_mode));
	else if ((sensor->cur_mode.bit_width == 12) && (sensor->cur_mode.index == IMX838_CLEAR_INDEX))
		err = imx838_write_reg_arry(
			sensor,
			(struct vvcam_sccb_data_s *)imx838_12bit_mode_clearHDR,
			ARRAY_SIZE(imx838_12bit_mode_clearHDR));
	else if (sensor->cur_mode.bit_width == 12)
		err = imx838_write_reg_arry(
			sensor,
			(struct vvcam_sccb_data_s *)imx838_12bit_mode,
			ARRAY_SIZE(imx838_12bit_mode));
	else {
...
static int imx838_s_stream(struct v4l2_subdev *sd, int enable)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct imx838 *sensor = client_to_imx838(client);
	int err = 0;
	...
	if (enable) {
		...
		imx838_write_reg(sensor, STANDBY, 0x00);
		msleep(30);
		imx838_write_reg(sensor, XMSTA, 0x00);
	} else  {
		pr_info("Disable stream\n");
		if (!(strcmp(sensor->gmsl, "gmsl")))
			max96792_stop_streaming(sensor->dser_dev,
						&sensor->i2c_client->dev);
		imx838_write_reg(sensor, STANDBY, 0x01);
		msleep(30);
		imx838_write_reg(sensor, XMSTA, 0x01);
	}
...
static int imx838_enum_mbus_code(struct v4l2_subdev *sd,
				...
				struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (code->index > 0)
		return -EINVAL;
	imx838_get_format_code(sensor, &cur_code);
	code->code = cur_code;

	return 0;
}

static int imx838_set_fmt(struct v4l2_subdev *sd,
			struct v4l2_subdev_state *state,
			struct v4l2_subdev_format *fmt)
{
	...
	imx838_get_format_code(sensor, &fmt->format.code);
	...
	ret = imx838_set_pixel_format(sensor);
	if (ret < 0) {
		pr_err("%s:imx838_write_reg_arry error, failed to set pixel format\n",
			__func__);
		mutex_unlock(&sensor->lock);
		return -EINVAL;
	}
	...
	switch (sensor->cur_mode.index)	{
	case IMX838_ALL_PIXEL_INDEX:
		pr_info("%s:Setting mode 0 ", __func__);
		ret = imx838_write_reg_arry(sensor, (struct vvcam_sccb_data_s *)mode_3856x2180, ARRAY_SIZE(mode_3856x2180));
		if (ret < 0) {
			pr_err("%s:imx838_write_reg_arry error, failed to set up resolution\n", __func__);
			return -EINVAL;
		}
		break;
	case IMX838_BINNING_INDEX:
		pr_info("%s:Setting mode 1 ", __func__);
		ret = imx838_write_reg_arry(sensor, (struct vvcam_sccb_data_s *)mode_h2v2_binning, ARRAY_SIZE(mode_h2v2_binning));
		if (ret < 0) {
			pr_err("%s:imx838_write_reg error, failed to set up resolution\n", __func__);
			return -EINVAL;
		}
		ret = imx838_set_data_rate(sensor, IMX838_720_MBPS);
		if (ret < 0) {
			pr_err("%s:imx838_write_reg_arry error, failed to set data rate\n", __func__);
			return -EINVAL;
		}
		break;
	case IMX838_DOL_INDEX:
		pr_info("%s:Setting mode 2 ", __func__);
		ret = imx838_write_reg_arry(sensor, (struct vvcam_sccb_data_s *)imx838_setting_dol_hdr, ARRAY_SIZE(imx838_setting_dol_hdr));
		if (ret < 0) {
			pr_err("%s:imx838_write_reg_arry error, failed to set up resolution\n", __func__);
			return -EINVAL;
		}
	...
	case IMX838_CLEAR_INDEX:
		...
		ret = imx838_write_reg_arry(sensor, (struct vvcam_sccb_data_s *)imx838_setting_clear_hdr, ARRAY_SIZE(imx838_setting_clear_hdr));
		if (ret < 0) {
			pr_err("%s:imx838_write_reg_arry error, failed to set up resolution\n", __func__);
			return -EINVAL;
		}
	...
	ret = imx838_s_ctrl(sensor->ctrls.data_rate);
	if (ret < 0) {
...
static int imx838_get_fmt(struct v4l2_subdev *sd,
			struct v4l2_subdev_state *state,
			struct v4l2_subdev_format *fmt)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static long imx838_priv_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct imx838 *sensor = client_to_imx838(client);
	long ret = 0;
	...
	case VIDIOC_QUERYCAP:
		ret = imx838_query_capability(sensor, arg);
		break;
	case VVSENSORIOC_QUERY:
		ret = imx838_query_supports(sensor, arg);
		break;
	case VVSENSORIOC_G_CHIP_ID:
		ret = imx838_get_sensor_id(sensor, arg);
		break;
	case VVSENSORIOC_G_RESERVE_ID:
		ret = imx838_get_reserve_id(sensor, arg);
		break;
	case VVSENSORIOC_G_SENSOR_MODE:
		ret = imx838_get_sensor_mode(sensor, arg);
		break;
	case VVSENSORIOC_S_SENSOR_MODE:
		ret = imx838_set_sensor_mode(sensor, arg);
		break;
	case VVSENSORIOC_S_STREAM:
		ret = imx838_s_stream(&sensor->sd, *(int *)arg);
		break;
	case VVSENSORIOC_WRITE_REG:
		ret = copy_from_user(&sensor_reg, arg,
			sizeof(struct vvcam_sccb_data_s));
		ret |= imx838_write_reg(sensor, sensor_reg.addr,
			sensor_reg.data);
		break;
	case VVSENSORIOC_READ_REG:
		ret = copy_from_user(&sensor_reg, arg,
			sizeof(struct vvcam_sccb_data_s));
		ret |= imx838_read_reg(sensor, sensor_reg.addr,
			(u8 *)&sensor_reg.data);
	...
	case VVSENSORIOC_S_EXP:
		ret = imx838_set_exp(sensor, *(u32 *)arg, 0);
		break;
	case VVSENSORIOC_S_VSEXP:
		ret = imx838_set_vs_exp(sensor, *(u32 *)arg, 0);
		break;
	case VVSENSORIOC_S_LONG_GAIN:
		ret = imx838_set_exp_gain(sensor, *(u32 *)arg, 0);
		break;
	case VVSENSORIOC_S_GAIN:
		ret = imx838_set_gain(sensor, *(u32 *)arg, 0);
		break;
	case VVSENSORIOC_S_VSGAIN:
		ret = imx838_set_vs_gain(sensor, *(u32 *)arg, 0);
		break;
	case VVSENSORIOC_S_FPS:
		ret = imx838_set_fps(sensor, *(u32 *)arg, 0);
		break;
	case VVSENSORIOC_G_FPS:
		ret = imx838_get_fps(sensor, (u32 *)arg);
		break;
	case VVSENSORIOC_S_HDR_RADIO:
		ret = imx838_set_ratio(sensor, arg);
		break;
	case VVSENSORIOC_S_BLC:
		ret = imx838_set_black_level(sensor, *(s64 *)arg, 0);
		break;
	...
	case VVSENSORIOC_S_TEST_PATTERN:
		ret = imx838_set_test_pattern(sensor, *(u32 *)arg);
		break;
	case VVSENSORIOC_S_DATA_RATE:
		ret = imx838_set_data_rate(sensor, *(u32 *)arg);
		break;
	case VVSENSORIOC_S_SYNC_MODE:
		ret = imx838_set_sync_mode(sensor, *(u32 *)arg);
		break;
	default:
		ret = -EINVAL;
		break;
...
static const struct v4l2_subdev_video_ops imx838_subdev_video_ops = {
	.s_stream = imx838_s_stream,
};

static const struct v4l2_subdev_pad_ops imx838_subdev_pad_ops = {
	.enum_mbus_code = imx838_enum_mbus_code,
	.set_fmt = imx838_set_fmt,
	.get_fmt = imx838_get_fmt,
};

static const struct v4l2_subdev_core_ops imx838_subdev_core_ops = {
	.s_power = imx838_s_power,
	.ioctl = imx838_priv_ioctl,
};

static const struct v4l2_subdev_ops imx838_subdev_ops = {
	.core  = &imx838_subdev_core_ops,
	.video = &imx838_subdev_video_ops,
	.pad   = &imx838_subdev_pad_ops,
};

static int imx838_link_setup(struct media_entity *entity,
...
static const struct media_entity_operations imx838_sd_media_ops = {
	.link_setup = imx838_link_setup,
};
...
static int imx838_probe(struct i2c_client *client)
{
	...
	err = imx838_parse_dt(sensor, client);
	if (err < 0) {
	...
	if (!(strcmp(sensor->gmsl, "gmsl"))) {
		...
		err = imx838_gmsl_serdes_setup(sensor);
		if (err) {
	...
	sd = &sensor->sd;
	v4l2_i2c_subdev_init(sd, client, &imx838_subdev_ops);
	sd->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	sd->dev = &client->dev;
	sd->entity.ops = &imx838_sd_media_ops;
	sd->entity.function = MEDIA_ENT_F_CAM_SENSOR;
	sensor->pads[IMX838_SENS_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	retval = media_entity_pads_init(&sd->entity,
				IMX838_SENS_PADS_NUM,
				sensor->pads);
	if (retval < 0)
		goto probe_err_power_off;

	memcpy(&sensor->cur_mode, &pimx838_mode_info[0],
		sizeof(struct vvcam_mode_info_s));
	...
	sensor->ctrls.exposure = v4l2_ctrl_new_std(&sensor->ctrls.handler, &imx838_ctrl_ops, V4L2_CID_EXPOSURE,
						3, 30000, 1, 1000);
	sensor->ctrls.gain = v4l2_ctrl_new_std(&sensor->ctrls.handler, &imx838_ctrl_ops, V4L2_CID_GAIN,
						0, 720, 3, 0);
	sensor->ctrls.black_level = v4l2_ctrl_new_std(&sensor->ctrls.handler, &imx838_ctrl_ops, V4L2_CID_BLACK_LEVEL,
						0, 1023, 1, 50);
	sensor->ctrls.data_rate = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_data_rate, NULL);
	sensor->ctrls.sync_mode = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_sync_mode, NULL);
	sensor->ctrls.framerate = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_framerate, NULL);

	sensor->ctrls.vs_exp = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_vs_exp, NULL);
	sensor->ctrls.vs_gain = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_vs_gain, NULL);
	sensor->ctrls.exp_gain = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_exp_gain, NULL);
	sensor->ctrls.conv_gain = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_conv_gain, NULL);
	sensor->ctrls.conv_vs_gain = v4l2_ctrl_new_custom(&sensor->ctrls.handler, imx838_ctrl_conv_vs_gain, NULL);

	sensor->ctrls.test_pattern = v4l2_ctrl_new_std_menu_items(&sensor->ctrls.handler, &imx838_ctrl_ops, V4L2_CID_TEST_PATTERN,
	...
	pr_info("%s camera mipi imx838, is found\n", __func__);
...
static int __maybe_unused imx838_suspend(struct device *dev)
{
	...
	if (sensor->resume_status)
		imx838_s_stream(&sensor->sd, 0);

	return 0;
}

static int __maybe_unused imx838_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct imx838 *sensor = client_to_imx838(client);

	if (sensor->resume_status)
		imx838_s_stream(&sensor->sd, 1);

	return 0;
}

static const struct dev_pm_ops imx838_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(imx838_suspend, imx838_resume)
};
...
static struct i2c_driver imx838_i2c_driver = {
	.driver = {
		...
		.name  = "imx838",
		.pm = &imx838_pm_ops,
		.of_match_table	= imx838_of_match,
	},
	.probe  = imx838_probe,
...
MODULE_DESCRIPTION("IMX838 MIPI Camera Subdev Driver");
MODULE_LICENSE("GPL");
